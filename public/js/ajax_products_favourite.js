    ...
    $.ajax({
     ...
     success:function(data)
     {
       ...
       if (data.isFave == true) {
         ...
         UIkit.notification({message: 'Producto agregado correctamente', pos: 'bottom-right', status:'primary',timeout:1300});
       }
       else {
         // le agregamos la clase correspondiente
         div_icon.attr('class',data.selected_class);
         UIkit.notification({message: 'Producto eliminado correctamente', pos: 'bottom-right',status:'danger',timeout:1300});
       }
